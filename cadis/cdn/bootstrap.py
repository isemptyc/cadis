...
def install_global_dataset(
    *,
    ...
    timeout_sec: int = 30,
    force_reinstall: bool = False,
    required_files: tuple[str, ...] = WATERBODY_REQUIRED_FILES,
    download_progress: Callable[[str, int, int | None], None] | None = None,
) -> dict[str, Any]:
    """Download and install a global (non-country) dataset such as waterbody.global."""
    cache_root = Path(cache_root).expanduser()

    root_manifest = read_json_url(dataset_manifest_url, timeout_sec=timeout_sec)
    global_datasets = root_manifest.get("global_datasets")
    if not isinstance(global_datasets, dict):
        raise ValueError("dataset_manifest.json missing global_datasets object.")

    entry = global_datasets.get(dataset_id)
    if not isinstance(entry, dict):
        raise ValueError(f"dataset_manifest.json has no global dataset entry for {dataset_id!r}.")

    latest = entry.get("latest")
    manifest_rel = entry.get("manifest")
    if not isinstance(latest, str) or not latest.strip():
        raise ValueError(f"global_datasets[{dataset_id!r}] missing latest version.")
    if not isinstance(manifest_rel, str) or not manifest_rel.strip():
        raise ValueError(f"global_datasets[{dataset_id!r}] missing manifest path.")

    target_dir = cache_root / "_global" / dataset_id / latest
    if (
        target_dir.exists()
        and not force_reinstall
        and not required_files_present(target_dir, required_files=required_files)
    ):
        return {
            "dataset_id": dataset_id,
    ...
    archive_block = entry.get("archive")
    if not isinstance(archive_block, dict):
        raise ValueError(f"global_datasets[{dataset_id!r}] missing archive block.")
    archive_rel = archive_block.get("path")
    archive_sha = archive_block.get("sha256")
    if not isinstance(archive_rel, str) or not isinstance(archive_sha, str):
        raise ValueError(f"global_datasets[{dataset_id!r}] archive missing path/sha256.")

    package_url = repo_relative_url(dataset_manifest_url, archive_rel)

    if force_reinstall and target_dir.exists():
        shutil.rmtree(target_dir)
    target_dir.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="cadis_global_pkg_") as tmp:
        tmp_path = Path(tmp)
        archive_path = tmp_path / "dataset_package.tar.gz"
        archive_path.write_bytes(
            read_bytes_url(
        ...
        if actual_sha != archive_sha:
            raise ValueError(
                f"Package checksum mismatch: expected={archive_sha} actual={actual_sha}"
            )
        safe_extract_tar_gz(archive_path, target_dir)

    missing = required_files_present(target_dir, required_files=required_files)
    if missing:
        raise ValueError(f"Extracted waterbody package missing required files: {missing}")

    local_manifest = target_dir / "dataset_release_manifest.json"
